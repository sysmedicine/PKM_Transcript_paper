#function:select cutoff, draw KM plot, calculate log-rank p value
...
#  SurvInput$EXP = EXP
  ...
  if (!hasArg("quantCut")) {
    ...
    Percentile20 = quantile(SurvInput$EXP,0.1)
    Percentile80 = quantile(SurvInput$EXP,0.9)
...
Cheng_generateSurvInputfromTCGA <- function(gene,TXT_name,dataDir) { 
  #if (!hasArg("dataDir")) {
  #  dataDir = paste0("C:/Pan_cancer/TCGA_transcript_result_store/",cancerType,"/PKLR/")
  #}
  data = Cheng_readFile(paste0(dataDir,TXT_name))
